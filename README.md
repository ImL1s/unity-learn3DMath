...
### 1. 座標變換的底層矩陣運算
雖然 Unity 封裝了 `Transform`，但底層仍然是 4x4 矩陣運算：
- **TRS 矩陣**：平移 (Translation)、旋轉 (Rotation)、縮放 (Scale) 如何結合成一個複合變換矩陣。
- **投影空間 (Projection Space)**：理解 GPU 如何將 3D 世界座標轉換為 2D 螢幕座標。本專案的 `02_Matrix` 有手動實現這些變換的範例。

### 2. 四元數與萬向鎖 (Gimbal Lock)
為什麼 Unity 建議內部使用 `Quaternion` 而非歐拉角？
- **萬向鎖**：當歐拉角的某兩個軸重合時會丟失一個自由度，導致旋轉異常。
- **球面線性插值 (Slerp)**：四元數提供了最平滑、最短路徑的旋轉插值。本專案的 `03_Quaternion` 展示了兩者的視覺差異。
